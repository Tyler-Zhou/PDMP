...
namespace PDMP.Application
{
    ...
    public class AuthorService: IAuthorService
    {
        ...
        /// <returns></returns>
        public async Task<AuthorInfo> AddAsync(AuthorInfo modelInfo)
        {
            var modelEntity = _Mapper.Map<AuthorEntity>(modelInfo);
            modelEntity.CreateDate = DateTime.Now;
            await _UnitOfWork.GetRepository<AuthorEntity>().InsertAsync(modelEntity);
            if (await _UnitOfWork.SaveChangesAsync() > 0)
                return _Mapper.Map<AuthorInfo>(modelEntity);
        ...
        {
            var repository = _UnitOfWork.GetRepository<AuthorEntity>();
        ...
        /// <returns></returns>
        public async Task<IPagedList<AuthorInfo>> GetPagedListAsync(QueryParameter parameter)
        {
            var repository = _UnitOfWork.GetRepository<AuthorEntity>();
            ...
            return _Mapper.Map<PagedList<AuthorInfo>>(modelPagedList);
        }
        /// <summary>
        /// 
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        public async Task<AuthorInfo> GetSingleAsync(long id)
        {
            var repository = _UnitOfWork.GetRepository<AuthorEntity>();
            var modelFind = await repository.GetFirstOrDefaultAsync(predicate: x => x.Id.Equals(id));
            return _Mapper.Map<AuthorInfo>(modelFind);
        }
        /// <summary>
        /// 
        /// </summary>
        /// <param name="modelInfo"></param>
        /// <returns></returns>
        public async Task<AuthorInfo> UpdateAsync(AuthorInfo modelInfo)
        {
            var modelEntity = _Mapper.Map<AuthorEntity>(modelInfo);
            var repository = _UnitOfWork.GetRepository<AuthorEntity>();
